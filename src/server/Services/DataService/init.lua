...
function DataService:HandleLeaderstats(player)
	...
	local highestWave = Instance.new("NumberValue")
	highestWave.Name = "HighestStage"
	highestWave.Value = 0
...
function DataService:KnitStart()
	...
	self.Client.TutorialCompleted:Connect(function(player)
		print'tut done'
		local replica = self:GetReplica(player)
		if not replica then return end 
		replica:SetValue("TutorialComplete", true)
	end)
end
